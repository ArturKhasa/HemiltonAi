...
# Тот же шаг, но опознанный по УШЕДШЕМУ тексту, а не по условию скрипта: по нему
# лестница статусов ставит «Ждем данные» (app.sales.status_flow). Условие скрипта
# для этого не годится — запрос контактов пишет и менеджер руками, а скрипт
# модель пересказывает своими словами. Общее у всех вариантов одно: ФИО и телефон
# просят вместе и в одном предложении.
ASKS_CONTACTS_RE = re.compile(
    r"(?:фио|ф\s*\.?\s*и\s*\.?\s*о)\b.{0,60}телефон"
    r"|телефон.{0,60}(?:фио|ф\s*\.?\s*и\s*\.?\s*о)\b",
    re.I | re.S,
)
