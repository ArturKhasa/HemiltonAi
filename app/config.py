...
class Settings(BaseSettings):
    ...
    # Ссылка на оплату. Живой менеджер выставлял счёт руками в платёжной системе,
    # генерации у нас пока нет — до неё сюда подставляется заглушка, чтобы воронку
    # можно было прогонять до конца. Домен example.com выбран нарочно: такую ссылку
    # невозможно перепутать с настоящей, если она случайно уйдёт клиенту.
    PAYMENT_LINK_URL: str = "https://example.com/pay/500"
