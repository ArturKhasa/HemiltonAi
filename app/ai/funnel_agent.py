...
STAGE_RULES: dict[str, str] = {
    ...
    "design": (
        "Перед тем как фиксировать дизайн, перечитай ВСЮ историю диалога и перечисли "
        "клиенту всё уже собранное: имя/фамилию для нанесения, изделие, цвет, размер, "
        "элементы дизайна. Ничего из названного клиентом не потеряй и не выдумай за него"
    ),
}
