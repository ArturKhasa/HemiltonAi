...
async def conversation_is_new(
    ...
    исходящие в расчёт не берём. Но у ручного сообщения ВК указывает
    ``admin_author_id``. Такое сообщение — это уже подключившийся менеджер,
    даже если клиент ответил через несколько секунд.
    ...
    for item in response.get("items") or []:
        ...
        if int(item.get("from_id") or 0) == int(vk_user_id):
            return False
        # Автоприветствие сообщества и сообщение менеджера приходят от одного
        # имени группы. Их отличает автор-администратор: при ручном ответе ВК
        # кладёт его id в admin_author_id. Нельзя списывать такой ответ на
        # свежее приветствие, иначе «всё верно, спасибо» получает реплику ИИ
        # поверх уже начатого менеджером диалога.
        if item.get("admin_author_id") is not None:
            return False
