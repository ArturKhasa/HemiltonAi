...
# Сумма в тексте. ОП пишет цены в скрипты руками и по-своему: «8.980р (вместо
# 12.980р)» в расчёте комплекта — точка разделителем тысяч и одна буква «р»
...
async def run_ai(
    ...
    # Клиент назвал надпись — дальше по регламенту идёт связка «Похвала →
    # Стоимость → Доставка», и цена должна уйти полноценным скриптом со своими
    # картинками. Модель вместо этого пересказывает всю связку одной строкой
    # («Супер, зафиксировала фамилию Шаманский! Свитшот … стоит 5 990 ₽. В какой
    # город планируете доставку?», диалог 351) — и код отступал, чтобы клиент не
    # прочитал цену дважды. Требование ОП от 17.08: «сообщение с ценой надо
    # отправлять жёстче в формате полноценного скрипта, а не ответа ИИ».
    #
    # Поэтому на этом шаге реплику модели заменяем скриптом целиком. Вопрос
    # клиента так не теряется: на вопрос отвечает модель, и связку в этом случае
    # достраивает блок ниже.
    if praise_point and "?" not in text:
        _praise = await find_praise_script(db, type_id)
        if _praise is not None and (_praise.phrase_text or "").strip():
            logger.info(
                "[%s] клиент назвал надпись — отдаём связку скриптом %s вместо пересказа",
                ctx, _praise.id,
            )
