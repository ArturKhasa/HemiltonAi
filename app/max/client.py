...
async def send_typing(token: str, chat_id: int) -> None:
    ...
    Эндпоинт проверен 08.09 запросом с заведомо несуществующим чатом (до
    живого клиента такой запрос дойти не может): MAX отвечает
    `{"success": true}` — значит и путь, и действие `typing_on` он принимает.

    Ошибку всё равно проглатываем: индикатор — украшение, из-за него ответ
    клиенту падать не должен.
    """
    try:
        await _request(
