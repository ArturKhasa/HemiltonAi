...
STAGE_LABELS: dict[str, str] = {
    ...
    "design": "Согласование раскладки нанесений + фиксация деталей",
    "checkout": "Способы оплаты + сбор ФИО и телефона для оформления",
    "payment_link": "Отправлена ссылка на оплату, ждём чек",
    # Вышивку в допродажу не предлагаем: «там сложные расчеты и большие цены»
    # (Лена, 30.07), и регламент прямо это запрещает. Строка её предлагала.
    "post_payment": "Чек получен — уточняем СДЭК/почту, возможен апсейл (принт/шрифт/второе изделие)",
...
STAGE_RULES: dict[str, str] = {
    ...
    # РОП: «Очень активно продаёт бежевые худи, второй раз мне предлагает обратить
    # внимание на них, хотя по цвету уже всё решили» (диалог 9, 30.07).
    "options": (
        "Как только клиент НАЗВАЛ цвет — он зафиксирован. НЕ предлагай другие цвета, "
        "НЕ возвращайся к выбору и НЕ уговаривай передумать, даже мимоходом и даже "
        "если другой цвет выигрышнее. Клиент сам захочет поменять — тогда меняй"
    ),
    # РОП: «В самом начале мы общались про дизайн... Дальше, когда мы дошли до
    # шаблона по дизайну, про фамилию забыли» (диалог 9, 30.07).
    #
    # Раньше здесь стояло «перечисли клиенту ВСЁ уже собранное» — и выходила
    # простыня на пять пунктов, которую клиент читал как допрос: «очень долго
    # согласование дизайна, негатив от клиента» (ОП, 21.08). Сверку отдаёт
    # скрипт с готовой раскладкой, модели остаётся не потерять названное.
    "design": (
        "Сверку дизайна отдавай раскладкой мест нанесения, а не пересказом: имя — "
        "на груди справа, фамилия — на спине с гербом, и только то, что клиент "
        "назвал сам. Ничего из названного не потеряй и не выдумай за него. "
        "Заново перечислять цвет, размер и цену не нужно — это уже согласовано"
    ),
}
